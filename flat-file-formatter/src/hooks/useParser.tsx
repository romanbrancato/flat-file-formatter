...
export function useParser() {
    ...
    const addField = useCallback((field: Record<string, unknown>) => {
        setIsReady(false);
        setData(fns.addField(data, field));
        setIsReady(true);
    }, [data, isReady]);

    const orderFields = useCallback((order: string[]) => {
        setIsReady(false);
        setData(fns.orderFields(data, order));
        setIsReady(true);
    }, [data, isReady]);

    const editHeader = useCallback((field: Record<string, string>) => {
        setIsReady(false);
        setData(fns.editHeader(data, field));
        setIsReady(true)
    }, [data, isReady]);

    const runFunction = useCallback((func: Function) => {
        setIsReady(false);
